...
def build_pattern(species: object, environmental_context: dict[str, Any] | None, *, water_context: dict[str, Any] | None = None, behavioral_context: dict[str, Any] | None = None, observed_context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    stage = infer_seasonal_stage(sid, date=(behavioral_context or {}).get("date"), air_temp_f=air.get("temp_f"), water_temp_f=water_data.get("temp_f"), air_temp_trend=air.get("air_temp_6h_delta"))
    ...
    return {"version": "v7.10-pattern-v1", "species": {"id": sid, "name": str(species or "Target species")}, "seasonal_state": {"stage": stage.get("stage"), "confidence": stage.get("confidence"), "estimated": bool(stage.get("estimated", True)), "evidence": stage.get("evidence_basis", [])}, "activity": activity, "feeding_mode": feeding, "forage": forage, "position": position, "presentation": presentation, "environment": {"daypart": (env.get("solar") or {}).get("daypart", "unknown"), "air_temp_f": air.get("temp_f"), "water_temp_f": water_data.get("temp_f"), "water_temp_source": water_data.get("temp_source", "unknown"), "wind_mph": air.get("wind_mph"), "wind_direction_deg": air.get("wind_direction_deg"), "cloud_pct": air.get("cloud_pct"), "pressure_trend": "falling" if (_num(air.get("pressure_6h_delta")) or 0) <= -0.05 else "rising" if (_num(air.get("pressure_6h_delta")) or 0) >= 0.05 else "stable_or_unknown", "rain_24h_in": air.get("rain_24h_in"), "flow_state": water_data.get("flow_state"), "clarity": (env.get("waterbody") or {}).get("clarity")}, "confidence": {"overall": overall, "score": round(activity["score"] * 0.55 + (70 if direct_water else 45) * 0.2 + (70 if position["structure"] else 40) * 0.25), "dimensions": dimensions}, "explanation": explanation[:5], "data_quality": {"direct_water_temp": direct_water, "direct_clarity": direct_clarity, "direct_forage": forage["observed"], "direct_structure": bool(observed.get("structure")), "direct_flow": direct_flow, "usgs_used": any(item.get("provider") == "usgs" for item in (env.get("provenance") or {}).values() if isinstance(item, dict)), "noaa_used": "noaa" in (env.get("provenance") or {}), "fallbacks_used": fallbacks}, "personal_evidence": {"live_applied": False, "status": "shadow_only"}}
